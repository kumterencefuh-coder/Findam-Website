...
function Logo() {
  return <span className="logo-mark"><span className="roof">⌂</span>Findam</span>
}
...
function SearchFields({ city, setCity, type, setType }) {
  return <>
    ...
    <label><span>Property type</span><select value={type} onChange={(e) => setType(e.target.value)}><option value="">Any</option><option>Rent</option><option>Sale</option><option>Short Let</option></select></label>
  </>
}
  ...
  return <>
    <header><div className="nav shell">
      ...
      <nav className={menuOpen ? 'open' : ''}>
        <button className={page === 'home' ? 'active' : ''} onClick={() => go('home')}>Home</button><button onClick={() => chooseType('Rent')}>Rent</button><button onClick={() => chooseType('Sale')}>Sell</button><button onClick={() => chooseType('Short Let')}>Short Let</button><button onClick={() => go('agents')}>Find Agents</button><button onClick={() => go('workflow')}>How it works</button>
      </nav>
    ...
    <footer><div className="shell footer"><div><Logo /><p>Find verified properties. Move in with confidence.</p></div><div><b>Explore</b><button onClick={() => chooseType('Rent')}>Rent a home</button><button onClick={() => go('agents')}>Find an agent</button></div><div><b>Available cities</b>{cities.map((item) => <span key={item}>{item}</span>)}</div></div></footer>
  </>
}
...
function Home({ city, setCity, type, setType, maxPrice, setMaxPrice, search, onOpen, savedIds, onSave, go }) { return <>
  <section className="hero"><div className="shell hero-grid"><div className="hero-copy">
    ...
    <div className="type-tabs"><button className={!type || type === 'Rent' ? 'selected' : ''} onClick={() => setType('Rent')}>For Rent</button><button className={type === 'Sale' ? 'selected' : ''} onClick={() => setType('Sale')}>For Sale</button><button className={type === 'Short Let' ? 'selected' : ''} onClick={() => setType('Short Let')}>Short Let</button></div>
  ...
  </div><div className="hero-visual"><div className="hero-photo" /><div className="verified-float">✓ <span>Agent visited<br />Engineer reviewed</span></div></div></div></section>
